...
list1 k = k >>= \k' -> list k >>= \ks -> pure (k' :. ks)
...
satisfy f =
  character >>= \c -> bool (unexpectedCharParser c) (valueParser c) (f c)
...
firstNameParser = upper >>= \a1 -> list lower >>= \a -> pure (a1 :. a)
...
surnameParser =
  upper >>= \a1 ->
    thisMany 5 lower >>= \a2 -> list lower >>= \a3 -> pure (a1 :. a2 ++ a3)
...
phoneParser =
  digit >>= \d -> phoneBodyParser >>= \z -> is '#' >> valueParser (d :. z)
...
personParser =
  ageParser >>= \a ->
    spaces1 >> firstNameParser >>= \firstname ->
      spaces1 >> surnameParser >>= \surname ->
        spaces1 >> smokerParser >>= \smoker ->
          spaces1 >> phoneParser >>=
            \phone -> pure (Person a firstname surname smoker phone)
