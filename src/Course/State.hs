...
firstRepeat l
  --eval (findM (\a -> State  (lift2 (lift2 (,)) S.member S.insert a) ) l) S.empty
 = eval (findM (\a -> State (\s -> (S.member a s, S.insert a s))) l) S.empty
...
distinct l =
  eval
    (filtering (\a -> State (\s -> (S.notMember a s, S.insert a s))) l)
    S.empty
