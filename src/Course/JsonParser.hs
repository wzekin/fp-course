...
jsonObject =
  betweenSepbyComma
    '{'
    '}'
    (jsonString >>= \s -> charTok ':' >> jsonValue >>= \j -> pure (s, j))
...
jsonValue =
  jsonString >>= \s ->
    pure (JsonString s) ||| jsonNull >>
    pure JsonNull |||
    (jsonNumber >>= \n -> pure (JsonRational n)) |||
    (jsonArray >>= \a -> pure (JsonArray a)) |||
    (jsonObject >>= \o -> pure (JsonObject o)) ||| jsonTrue >>
    pure JsonTrue ||| jsonFalse >>
    pure JsonFalse
...
readJsonValue filePath =
  readFile filePath >>= \content -> pure (parse jsonValue content)
